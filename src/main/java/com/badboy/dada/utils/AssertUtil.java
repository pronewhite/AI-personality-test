...
public class AssertUtil {
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(ExceptionResponseEnum.PARAM_INVALID, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
    ...
    public static void isEmpty(Object obj, ErrorEnum errorEnum) {
        if (!isEmpty(obj)) {
            throwException(errorEnum.getErrorMessage());
        }
    }
    ...
    private static void throwException(ErrorEnum errorEnum, Object... arg) {
        if (Objects.isNull(errorEnum)) {
            errorEnum = ExceptionResponseEnum.BUISSNESS_EXCEPTION;
        }
